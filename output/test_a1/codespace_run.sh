...
if [ ! -f "${SOURCE_MP4}" ]; then
  echo "ERROR: ${SOURCE_MP4} が見つかりません。" >&2
  echo "ローカル PC で local_fetch.sh を実行し、" >&2
...
if [ -f "${SRT_PATH}" ]; then
  ...
  echo "WARN: ${SRT_PATH} なし。字幕なしで進めますが切り出しは行われません。" >&2
fi
...
rm -rf "${EXTRACT_DIR}/parts" "${EXTRACT_DIR}/cut.sh" "${EXTRACT_DIR}/combine.sh" "${EXTRACT_DIR}/concat.txt" "${EXTRACT_DIR}/download.sh" "${EXTRACT_DIR}/manifest.json" 2>/dev/null || true
